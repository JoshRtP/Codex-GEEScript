...
var USDA_ENTROPY_PALETTE = [
  '#000080', '#0000D9', '#4000FF', '#8000FF', '#0080FF', '#00FFFF',
  '#00FF80', '#80FF00', '#DAFF00', '#FFFF00', '#FFF500', '#FFDA00',
  '#FFB000', '#FFA400', '#FF4F00', '#FF2500', '#FF0A00', '#FF00FF'
];
var USDA_ENTROPY_VIS = {min: 0, max: 2, palette: USDA_ENTROPY_PALETTE};
...
uiPanel.add(ui.Label('USDA Full Feature - Field Analytics (S2 + S1 + MODIS + CDL) + Covariates', {
...
usdaPanel.add(ui.Label('USDA Full Feature Layers', {fontWeight:'bold', fontSize:'12px', color:'#143a52'}));
...
function usdaSentinel2Rgb(startDateText, endDateText, cloudFilter, geom){
  ...
  var imgCol = ee.ImageCollection('COPERNICUS/S2_SR')
    .filterBounds(geom)
    .filterDate(start, end)
    .filter(ee.Filter.lt('CLOUD_COVERAGE_ASSESSMENT', cloudFilter));
...
function usdaEntropyFromCollection(cdlCol){
  var numberOfCdl = cdlCol.count().rename('cdl_count');
  var cropAutoHistogram = cdlCol.reduce(ee.Reducer.autoHistogram());
  var cropFrequency = cropAutoHistogram.arraySlice(1, 1, 2).rename('frequency');
  var probability = cropFrequency.divide(numberOfCdl);
  var probabilityReciprocal = ee.Image(1).divide(probability).rename('p_reciprocal');
  var logProbabilityReciprocal = probabilityReciprocal.log().rename('log_p_reciprocal');
  var product = probability.multiply(logProbabilityReciprocal).rename('product');
  product = product.arrayMask(product.eq(product));
  return product
    .arrayReduce(ee.Reducer.sum(), [0])
    .arrayProject([0])
    .arrayFlatten([['entropy']])
    .updateMask(numberOfCdl.gt(0));
}
function usdaTemporalEntropy(endYear, geom){
  var y = ee.Number(endYear).int().max(ENTROPY_FIRST_YEAR).min(CDL_LAST);
  var cdlCol = ee.ImageCollection('USDA/NASS/CDL')
    .select('cropland')
    .filterDate(ee.Date.fromYMD(ENTROPY_FIRST_YEAR, 1, 1), ee.Date.fromYMD(y, 12, 31).advance(1, 'day'))
    .filterBounds(geom)
    .map(function(img){ return img.clip(geom); });

  return ee.Image(ee.Algorithms.If(
    cdlCol.size().gt(0),
    usdaEntropyFromCollection(cdlCol),
    emptyBands(['entropy'])
  )).rename('entropy').clip(geom);
}
...
function tillageEventMaskComposite(startDateText, endDateText, cloudPct, geom, ndtiMax, ndviMax, ndmiMax){
  ...
  var comp = ee.Image(ee.Algorithms.If(
    s2.size().gt(0),
    s2.median().select(['NDTI', 'NDVI', 'NDMI']),
    emptyBands(['NDTI', 'NDVI', 'NDMI'])
  )).clip(geom);
...
function selectedGeomForUsdaLayers(){
  if (!state.lastGeom) {
    statusLabel.setValue('Click a field first.');
    return null;
  }
...
toggleUsdaS2Btn.onClick(function(){
  ...
  if (!geom) { return; }
  if (state.showUsdaS2Layer){
    removeLayerByName(USDA_S2_LAYER_NAME);
    state.showUsdaS2Layer = false;
    statusLabel.setValue('USDA Sentinel-2 layer removed.');
    return;
  }
  ...
  if (!isValidIsoDate(startText) || !isValidIsoDate(endText)){
    statusLabel.setValue('Use YYYY-MM-DD for Sentinel-2 start/end.');
    return;
  }
  if (new Date(startText + 'T00:00:00Z').getTime() > new Date(endText + 'T00:00:00Z').getTime()){
    statusLabel.setValue('Sentinel-2 date range invalid: start must be <= end.');
    return;
  }
  var cloudPct = Number(usdaS2CloudSlider.getValue());
  Map.addLayer(
    usdaSentinel2Rgb(startText, endText, cloudPct, geom),
    {bands:['N','R','G'], min:0.01, max:0.5},
    USDA_S2_LAYER_NAME,
    true,
    0.9
  );
  state.showUsdaS2Layer = true;
  statusLabel.setValue('USDA Sentinel-2 layer added (' + startText + ' to ' + endText + ').');
});

toggleEntropyBtn.onClick(function(){
  var geom = selectedGeomForUsdaLayers();
  if (!geom) { return; }
  if (state.showEntropyLayer){
    removeLayerByName(USDA_ENTROPY_LAYER_NAME);
    state.showEntropyLayer = false;
    statusLabel.setValue('USDA temporal entropy layer removed.');
    return;
  }
  var entropyYear = parseInt((entropyYearBox.getValue() || '').trim(), 10);
  if (isNaN(entropyYear)){
    statusLabel.setValue('Entropy year must be numeric (e.g., 2024).');
    return;
  }
  if (entropyYear < ENTROPY_FIRST_YEAR) { entropyYear = ENTROPY_FIRST_YEAR; }
  if (entropyYear > CDL_LAST) { entropyYear = CDL_LAST; }
  entropyYearBox.setValue(String(entropyYear));
  Map.addLayer(
    usdaTemporalEntropy(entropyYear, geom),
    USDA_ENTROPY_VIS,
    USDA_ENTROPY_LAYER_NAME,
    true,
    0.9
  );
  state.showEntropyLayer = true;
  statusLabel.setValue('USDA temporal entropy layer added (2008 to ' + entropyYear + ').');
});

toggleNdtiBtn.onClick(function(){
  var geom = selectedGeomForUsdaLayers();
  if (!geom) { return; }
  if (state.showNdtiLayer){
    removeLayerByName(NDTI_LAYER_NAME);
    state.showNdtiLayer = false;
    statusLabel.setValue('NDTI overlay removed.');
    return;
  }
  var startText = (usdaS2StartBox.getValue() || '').trim();
  var endText = (usdaS2EndBox.getValue() || '').trim();
  if (!isValidIsoDate(startText) || !isValidIsoDate(endText)){
    statusLabel.setValue('Use YYYY-MM-DD for NDTI start/end.');
    return;
  }
  if (new Date(startText + 'T00:00:00Z').getTime() > new Date(endText + 'T00:00:00Z').getTime()){
    statusLabel.setValue('NDTI date range invalid: start must be <= end.');
    return;
  }
  var cloudPct = Number(usdaS2CloudSlider.getValue());
  Map.addLayer(
    ndtiComposite(startText, endText, cloudPct, geom),
...
toggleTillageEventBtn.onClick(function(){
  var geom = selectedGeomForUsdaLayers();
  if (!geom) { return; }
  if (state.showTillageEventLayer){
    removeLayerByName(TILL_EVENT_LAYER_NAME);
    state.showTillageEventLayer = false;
    statusLabel.setValue('Likely tillage event mask removed.');
    return;
  }
  var startText = (usdaS2StartBox.getValue() || '').trim();
  var endText = (usdaS2EndBox.getValue() || '').trim();
  if (!isValidIsoDate(startText) || !isValidIsoDate(endText)){
    statusLabel.setValue('Use YYYY-MM-DD for tillage-event start/end.');
    return;
  }
  if (new Date(startText + 'T00:00:00Z').getTime() > new Date(endText + 'T00:00:00Z').getTime()){
    statusLabel.setValue('Tillage-event date range invalid: start must be <= end.');
    return;
  }
  var cloudPct = Number(usdaS2CloudSlider.getValue());
  ...
  Map.addLayer(
    eventMask.updateMask(eventMask),
    {palette:['#ff00ff']},
    TILL_EVENT_LAYER_NAME,
    true,
    0.95
  );
  state.showTillageEventLayer = true;
  statusLabel.setValue(
    'Likely tillage event mask added. NDTI<=' + proxyFmt(ndtiMax, 2) +
    ', NDVI<=' + proxyFmt(ndviMax, 2) +
    ', NDMI<=' + proxyFmt(ndmiMax, 2) + '.'
  );
});
...
function addAnalysisPanel(title, lines){
  ...
  for (var i = 0; i < lines.length; i++) {
    pop.add(ui.Label(lines[i], {fontSize:'11px', whiteSpace:'normal'}));
  }
}
...
function runCoverCropAnalysis(){
  ...
  merged.evaluate(function(stats, error){
    ...
    addAnalysisPanel('Cover Crop Analysis', [
      ...
      'Trend (# = cover crop present, . = absent):',
      (state.mgmtYears || []).map(function(y){
        var v = Number(stats['cover_crop_likely_' + y]);
        return y + ':' + (isNaN(v) ? '?' : (v >= 0.5 ? '#' : '.'));
      }).join('  ')
    ]);
